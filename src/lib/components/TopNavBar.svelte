...
>
	<div class="flex justify-between items-center w-full px-margin-mobile md:px-margin-desktop py-4 relative z-50 bg-surface">
		<div
		...
		>
			<div class="w-5 h-[2px] bg-black transition-all duration-300 {menuOpen ? 'rotate-45 translate-y-[6px]' : ''}"></div>
			<div class="w-5 h-[2px] bg-black transition-all duration-300 {menuOpen ? 'opacity-0 scale-x-0' : 'opacity-100 scale-x-100'}"></div>
			<div class="w-5 h-[2px] bg-black transition-all duration-300 {menuOpen ? '-rotate-45 -translate-y-[6px]' : ''}"></div>
		</button>
	</div>
	...
	{#if menuOpen}
		...
		>
			<a 
				class="{isActive('/') ? 'bg-primary text-on-primary border-black shadow-cartoon' : 'text-on-surface border-transparent'} border-2 px-4 py-3 font-nav-lg text-nav-lg uppercase tracking-widest transition-all text-center" 
				href="/" 
				on:click={(e) => handleLinkClick(e, '/')}>HOME</a
			>
			<a 
				class="{isActive('/experience') ? 'bg-primary text-on-primary border-black shadow-cartoon' : 'text-on-surface border-transparent'} border-2 px-4 py-3 font-nav-lg text-nav-lg uppercase tracking-widest transition-all text-center" 
				href="/experience" 
				on:click={(e) => handleLinkClick(e, '/experience')}>EXPERIENCE</a
			>
			<span 
				class="text-on-surface/50 px-4 py-3 font-nav-lg text-nav-lg uppercase tracking-widest cursor-not-allowed text-center border-2 border-transparent"
				>BLOG</span
			>
