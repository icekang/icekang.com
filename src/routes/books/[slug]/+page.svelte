<script lang="ts">
	...
	let coverUrl = `https://covers.openlibrary.org/b/isbn/${book.isbn}-L.jpg`;
	let coverSource = 'Open Library';
	...
	function handleImageError() {
		// No fallback needed for now as OL is the primary
	}
...
<div
	class="fixed top-0 left-0 w-full h-1 bg-white border-b border-black z-[60]"
	id="reading-progress"
>
	<div class="relative w-full h-full">
		<div class="absolute bottom-0 flex flex-col items-center mb-0" style="left: {scrollProgress}%">
			<div class="flex items-center gap-2 mb-1 transform -translate-x-1/2">
				<span
					class="font-label-md text-[12px] font-bold uppercase whitespace-nowrap bg-white px-1 border border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
				>
			...
			<div
				class="w-full bg-surface-accent h-1 transition-all duration-150"
...
>
	...
	<main class="flex-grow flex flex-col w-full">
		...
		<header class="w-full border-b-2 border-black flex flex-col md:flex-row">
			...
			<div
				class="w-full md:w-1/2 bg-surface-accent border-l-0 md:border-l-2 border-black relative min-h-[300px] md:min-h-[500px] flex items-center justify-center overflow-hidden"
			>
				<img
					...
					style="transform: translateY({y * (isMobile ? -0.2 : 0.35)}px) scale(1.1);"
				/>
				<!-- Decorative Elements -->
				<div
					class="absolute top-8 left-8 bg-white border-2 border-black p-2 shadow-cartoon transform -rotate-3 z-20"
					style="transform: translateY({y * -0.2}px) rotate(-3deg);"
				>
				...
				<div
					...
					style="transform: translateY({y * 0.1}px) rotate(6deg);"
				>
		...
		<section class="w-full flex flex-col md:flex-row flex-grow">
			<!-- Metadata Sidebar -->
			<aside
				class="w-full md:w-[300px] flex-shrink-0 border-b-2 md:border-b-0 md:border-r-2 border-black p-margin-mobile md:p-margin-desktop flex flex-col gap-8 bg-surface-main"
			>
				<div class="flex flex-col gap-2">
					<h3
						class="font-label-caps text-label-caps text-surface-accent border-b-2 border-black pb-1 uppercase inline-block self-start"
					>
						Author
					</h3>
					...
					<div class="flex gap-1">
						{#each Array(5) as _, i}
							<span
								...
								style="font-variation-settings: 'FILL' {i < book.rating ? 1 : 0};">star</span
							>
						{/each}
					</div>
				</div>
				...
				<div class="flex flex-col gap-2">
					...
					<div class="flex flex-wrap gap-2 mt-2">
						<span
							class="border-2 border-black px-2 py-1 bg-white font-label-md text-label-md shadow-cartoon-sm"
						>
				...
				<div class="flex flex-col gap-2">
					...
					<p class="font-body-md text-body-md italic opacity-70">
						{coverSource}
					</p>
				</div>
				<div class="mt-auto pt-8">
					<a
						...
						class="w-full bg-surface-accent text-white border-2 border-black py-3 font-label-caps text-label-caps shadow-cartoon hover:scale-[0.98] transition-transform flex items-center justify-center gap-2"
					>
			...
			<article
				...
				class="w-full p-margin-mobile md:p-margin-desktop max-w-4xl bg-white relative"
			>
				...
				<div class="space-y-6 font-body-lg text-body-lg leading-relaxed">
					{#each book.fullReview.paragraphs as paragraph, i}
						<p
							class={i === 0
								? 'first-letter:text-7xl first-letter:font-bold first-letter:text-surface-accent first-letter:mr-3 first-letter:float-left'
								: ''}
						>
							{paragraph}
						</p>
					...
					<div
						class="my-12 p-8 border-2 border-black bg-surface-main shadow-cartoon relative transform rotate-1 hover:rotate-0 transition-transform"
					>
					...
					<h3
						class="font-headline-md text-2xl font-bold mt-12 mb-6 border-b-2 border-black inline-block pb-2"
					>
